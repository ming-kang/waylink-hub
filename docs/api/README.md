...
| GET | `/api/devices/status/query/<device_id>/` | 获取状态查询指令（轮询） | API Key |
| GET | `/api/devices/query/<device_id>/` | 管理员查询柜子状态 | JWT (Admin) |
...
上报柜门状态、锁状态、物品检测状态和电量。柜子状态变化时立即上报。
...
| 参数 | 类型 | 说明 |
|-----|------|------|
| cabinet_status | object | 柜子状态详情，详见下方说明 |
| battery_level | int | 电量百分比（0-100） |
...
{
    "cabinet_id": {
        "door": true,        // true=关闭, false=开启（红外距离传感器）
        "lock_angle": 0,     // 锁舌角度（0-360度）
        "lock_locked": true, // true=锁定, false=解锁（电机状态）
        "has_item": false    // true=有物品, false=无物品（红外传感器）
    }
}
```
...
**GET** `/api/devices/status/query/<device_id>/`

设备每 5 秒轮询一次，检查是否有待执行的状态查询指令。
...
```json
{
    ...
    "data": {
        ...
        "cabinet_ids": ["A001", "A002"],
        "timestamp": "2026-01-20T10:30:23.874651+08:00"
    }
}
```
...
> ESP32 收到查询指令后，应立即读取各传感器数据并调用 `/api/devices/status/` 上报

---

### 管理员查询柜子状态

**GET** `/api/devices/query/<device_id>/`

管理员主动查询设备柜子状态，服务器会向设备下发查询指令。
...
```json
{
    ...
    "data": {
        "device_id": "ESP32_001",
        "status": "online",
        "last_heartbeat": "2026-01-20T10:35:00+08:00",
        "cabinets": [
            {
                "cabinet_id": "A001",
                "door_closed": true,
                ...
                "lock_locked": true,
                "has_item": true,
                "item_detected_at": "2026-01-20T10:30:00+08:00",
                "last_updated": "2026-01-20T10:35:00+08:00"
            }
        ]
    }
}
...
| 字段 | 类型 | 说明 | 数据来源 |
|------|------|------|----------|
| door | boolean | 柜门状态：true=关闭，false=开启 | 红外距离传感器 |
| lock_angle | int | 电机锁舌角度（0-360度） | 步进电机编码器 |
| lock_locked | boolean | 电机锁状态：true=锁定，false=解锁 | 电机驱动电路 |
| has_item | boolean | 柜内物品检测：true=有物品，false=无物品 | 红外对射传感器 |
| item_detected_at | datetime | 最后检测到物品变化的时间 | - |
